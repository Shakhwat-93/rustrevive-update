  ...
  return (
    ...
    >
      <div className="space-y-6">
        ...
        {/* 3. Reviews Table & Listing (Responsive) */}
        <div className="bg-white border border-slate-200 rounded-xl shadow-2xs overflow-hidden">
          {loading ? (
            ...
            <>
              ...
              <div className="hidden md:block overflow-x-auto">
                <table className="w-full text-left text-xs min-w-[900px]">
                  <thead className="bg-slate-50 border-b border-slate-200 text-slate-500 font-mono uppercase tracking-wider text-[10px]">
                    <tr>
                      <th className="py-3 px-4 w-[240px]">Garment / Product</th>
                      <th className="py-3 px-4 w-[160px]">Reviewer</th>
                      <th className="py-3 px-4 w-[120px]">Rating</th>
                      <th className="py-3 px-4 min-w-[220px]">Feedback Details</th>
                      <th className="py-3 px-4 w-[110px]">Status</th>
                      <th className="py-3 px-4 w-[110px]">Date</th>
                      <th className="py-3 px-4 text-right w-[140px]">Moderation</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-100">
                    {reviews.map((row) => {
                      const primaryMedia =
                        row.products?.product_media?.find((m) => m.is_primary) || row.products?.product_media?.[0];
                        ...
                        >
                          ...
                          <td className="py-3.5 px-4">
                            <div className="space-y-0.5">
                              <p className="font-semibold text-slate-900 truncate max-w-[140px]">{row.customer_name}</p>
                              {row.is_verified_purchase ? (
                                <span className="inline-flex items-center text-[9px] font-mono font-bold text-emerald-700 bg-emerald-50 px-1.5 py-0.5 rounded border border-emerald-200 whitespace-nowrap">
                                  <ShieldCheck className="w-2.5 h-2.5 mr-0.5" />
                                  Verified Buyer
                                </span>
                              ) : (
                                <span className="text-[10px] text-slate-400 font-mono whitespace-nowrap">Store Patron</span>
                              )}
                            </div>
                          </td>
                          ...
                          {/* Feedback Details */}
                          <td className="py-3.5 px-4">
                            {row.title && <p className="font-bold text-slate-900 truncate max-w-xs">{row.title}</p>}
                            <p className="text-slate-600 line-clamp-2 mt-0.5 text-[11px] leading-relaxed max-w-sm">
                              {row.content}
                            </p>
                          </td>
              ...
              {/* --- MOBILE CARDS VIEW (under md screen) --- */}
                    ...
                    >
                      ...
                      <div className="flex items-start justify-between gap-2.5">
                        <div className="flex items-center space-x-3 min-w-0">
                          <div className="relative w-11 h-13 bg-slate-100 border border-slate-200 rounded-sm overflow-hidden shrink-0">
                            <Image
                              src={imgUrl}
                              alt={row.products?.title || "Product"}
                              fill
                      ...
                      <div className="flex items-center justify-between gap-2 pt-1 border-t border-slate-100/70 text-xs">
                        <div className="flex items-center space-x-2 min-w-0">
                          <span className="font-bold text-slate-900 truncate max-w-[130px]">{row.customer_name}</span>
                          {row.is_verified_purchase && (
                            <span className="inline-flex items-center text-[9px] font-mono font-bold text-emerald-700 bg-emerald-50 px-1 py-0.2 rounded border border-emerald-200 shrink-0">
                              <ShieldCheck className="w-2.5 h-2.5 mr-0.5" />
                              Verified
                            </span>
                          )}
                        ...
                        <div className="flex items-center space-x-1">
                          {row.status !== "APPROVED" && (
                            <button
                              ...
                              disabled={actionLoadingId === row.id}
                              onClick={() => handleModerate(row.id, "APPROVED")}
                              className="px-2.5 py-1 text-[11px] font-medium text-emerald-700 bg-emerald-50 hover:bg-emerald-100 rounded-md border border-emerald-200 flex items-center space-x-1 cursor-pointer transition-colors"
                            >
                          ...
                          {row.status !== "REJECTED" && (
                            <button
                              ...
                              disabled={actionLoadingId === row.id}
                              onClick={() => handleModerate(row.id, "REJECTED")}
                              className="px-2.5 py-1 text-[11px] font-medium text-rose-700 bg-rose-50 hover:bg-rose-100 rounded-md border border-rose-200 flex items-center space-x-1 cursor-pointer transition-colors"
                            >
      ...
      {inspectReview && (
        <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-xs flex items-center justify-center p-3 sm:p-4">
          <div className="bg-white border border-slate-200 rounded-2xl max-w-lg w-full max-h-[90vh] overflow-y-auto p-5 sm:p-6 shadow-2xl space-y-5 relative animate-fade-in">
            <button
              type="button"
